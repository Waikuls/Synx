...
- ถ้าผู้ใช้ขอเพิ่มเมนู, ปุ่ม, dropdown, slider, toggle หรือ section: ให้แยก UI ตามเมนูไว้ใน `ui/` ตั้งแต่แรกเมื่อเมนูนั้นมีแนวโน้มจะมีหลายฟีเจอร์
- `Fatality/main.lua` ควรเป็นไฟล์รวมระบบและตัวเรียกใช้ modules มากกว่าจะเป็นที่เก็บรายละเอียดของทุกเมนู
- ถ้าผู้ใช้ขอให้ฟีเจอร์ใช้งานได้จริง: แยก UI ของเมนูไว้ใน `ui/<menu>.lua` และแยก logic ไว้ใน `features/<feature>.lua` ตามความเหมาะสม
...
## กฎการสร้างไฟล์ใหม่
- ถ้าฟีเจอร์มี state, loop, drawing, player tracking, cleanup, live updates, หรือ logic ยาว ให้สร้างไฟล์ใหม่ใน `features/`
- ถ้าเมนูใดใน UI มีแนวโน้มจะมีหลายฟีเจอร์ ให้แยก UI ของเมนูนั้นไปไว้ใน `ui/` ตั้งแต่ต้น ไม่ต้องรอให้ไฟล์ยาวก่อน
- `main.lua` ควรเป็นตัวรวมระบบ โหลด library, feature modules, และ ui modules ไม่ควรเก็บรายละเอียดของทุกเมนูไว้ทั้งหมด
- ฟีเจอร์ใหม่ที่มีทั้ง UI และ logic ควรแยกอย่างน้อย 2 ส่วนเมื่อเหมาะสม:
- `ui/<menu>.lua` สำหรับสร้างเมนู
- `features/<feature>.lua` สำหรับ logic ของฟีเจอร์
- ถ้าเป็นการแก้เล็กน้อย เช่นเปลี่ยนชื่อเมนู, เพิ่มปุ่มสั้น ๆ, เพิ่ม callback สั้น ๆ ยังไม่จำเป็นต้องสร้างไฟล์ใหม่
- ห้ามสร้างไฟล์ใหม่ถ้ายังไม่มีหน้าที่ชัดเจนหรือทำให้โครงสร้างซ้ำซ้อนโดยไม่ช่วยให้อ่านง่ายขึ้น

## กฎ GitHub
- ถ้าผู้ใช้สั่งให้ `push` ขึ้น GitHub หลังแก้งานเสร็จ ให้ทำ `git add`, `git commit`, และ `git push` ต่อทันที ไม่ต้องรอให้ผู้ใช้ย้ำหลายครั้ง
- ถ้ามีไฟล์ที่แก้ค้างอยู่และเกี่ยวข้องกับงานรอบนั้น ให้รวมขึ้น GitHub ในรอบเดียวกัน
- ถ้างานรอบนั้นเป็นการแก้โค้ดจริงและไม่มีคำสั่งให้หยุดไว้ก่อน ให้ `push` ขึ้น GitHub อัตโนมัติหลังแก้เสร็จ ทดสอบเท่าที่ทำได้ และสรุปผลแล้ว โดยไม่ต้องรอให้ผู้ใช้สั่งซ้ำ
- ถ้ามีทั้งไฟล์ logic, UI, หรือ config ที่เกี่ยวข้องกับงานเดียวกันและพร้อมขึ้น repo แล้ว ให้รวม `commit` และ `push` ในรอบเดียว ไม่แยกย่อยโดยไม่จำเป็น
